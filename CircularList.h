...
template <typename T>
CircularList<T>::CircularList(const CircularList& other) : head(nullptr), count(0) {
...
bool CircularList<T>::operator<(const CircularList& other) const {
    ...
    while (it1 != end() && it2 != other.end() && elements_compared < std::min(size(), other.size())) {
...
typename CircularList<T>::iterator CircularList<T>::erase(iterator pos) {
    ...
    if (pos.node == nullptr) throw std::invalid_argument("CircularList::erase: invalid iterator");
    Node* node = pos.node;
...
template <typename T>
CircularList<T>::const_iterator::const_iterator(Node* n, Node* h) : node(n), head(h) {}

template <typename T>
const T& CircularList<T>::const_iterator::operator*() const {
    if (!node) throw std::out_of_range("CircularList::const_iterator::operator*: dereferencing end iterator");
    return node->data;
}

template <typename T>
typename CircularList<T>::const_iterator& CircularList<T>::const_iterator::operator++() {
    if (!node) throw std::out_of_range("CircularList::const_iterator::operator++: incrementing end iterator");
    node = node->next;
    return *this;
}
...
template <typename T>
typename CircularList<T>::const_iterator& CircularList<T>::const_iterator::operator--() {
    if (!head) throw std::out_of_range("CircularList::const_iterator::operator--: no list");
    if (!node) {
        node = head->prev;
    } else {
        node = node->prev;
...
T& CircularList<T>::iterator::operator*() {
    if (!node) throw std::out_of_range("CircularList::iterator::operator*: dereferencing end iterator");
    return node->data;
}

template <typename T>
typename CircularList<T>::iterator& CircularList<T>::iterator::operator++() {
    if (!node) throw std::out_of_range("CircularList::iterator::operator++: incrementing end iterator");
    node = node->next;
    return *this;
}
...
template <typename T>
typename CircularList<T>::iterator& CircularList<T>::iterator::operator--() {
    if (!head) throw std::out_of_range("CircularList::iterator::operator--: no list");
    if (!node) {
        node = head->prev;
    } else {
        node = node->prev;
